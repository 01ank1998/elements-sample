...
import { MODULE_PATHS } from './module-paths';

@Injectable({
  providedIn: 'root',
})
export class LazyLoadModuleService {
  private componentsToLoad: Map<string, any>;
  private loadedComponents = new Map<string, Type<any>>();
  private elementsLoading = new Map<string, Promise<any>>();
  ...
  constructor(private compiler: Compiler, private injector: Injector) {
    ...
    const modulePaths = MODULE_PATHS;
  ...
  receiveContext(component: Type<any>, injector: Injector) {
    ...
    this.componentFactoryResolvers.set(
      component,
  ...
  getComponentToLoad(selector: string): Observable<any> {
   ...
   */
  loadComponent(componentSelector: string): Promise<any> {
            ...
            if (
              ...
              if (!componentClass) {
                ...
                throw `You specified multiple component elements in module ${elementModuleRef} but there was no match for tag
                        ${componentSelector} in ${JSON.stringify(
                  elementModuleRef.instance.customElementComponent
                )}.
